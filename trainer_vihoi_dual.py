...
from manip.data.cano_traj_dataset_dual import CanoObjectTrajDataset, quat_ik_torch, quat_fk_torch#, collate_fn_with_vlm_padding
from manip.data.long_cano_traj_dataset import LongCanoObjectTrajDataset 
from manip.data.unseen_obj_long_cano_traj_dataset_dual import UnseenCanoObjectTrajDataset 

from manip.model.transformer_object_motion_cond_diffusion_dual import ObjectCondGaussianDiffusion 
...
# logger.remove() 
from Qwen.tools.utils import makepath, LOGGER_DEFAULT_FORMAT
import time

def cfg_logger(logger_path):
    logger.remove()  # 移除默认控制台输出
    logger.add(logger_path, backtrace=True, diagnose=True)  # 写入到文件
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        # Load pretrained CHOIS model if specified
        self.freeze_chois_and_train_projection = opt.freeze_chois_and_train_projection
        if hasattr(opt, 'pretrained_chois_path') and opt.pretrained_chois_path:
            self.load_pretrained_chois(opt.pretrained_chois_path)
        
        # Set up optimizer based on freeze option
        if self.freeze_chois_and_train_projection:
            # Freeze all CHOIS parameters except vlm_token_projector and vlm_aggregator
            self.freeze_chois_parameters()
            # Only optimize VLM projector components (vlm_token_projector + vlm_aggregator)
            trainable_params = filter(lambda p: p.requires_grad, diffusion_model.parameters())
            self.optimizer = Adam(trainable_params, lr=train_lr)
            print("Training mode: Only VLM projector components will be trained")
        else:
            # Train all parameters
            self.optimizer = Adam(diffusion_model.parameters(), lr=train_lr)
            print("Training mode: All model parameters will be trained")
        ...
        self.use_vlm_condition = self.opt.use_vlm_condition 

        self.use_dual_condition = self.opt.use_dual_condition
        ...
        if self.test_unseen_objects:
            ...
            self.unseen_seq_ds = UnseenCanoObjectTrajDataset(train=False, \
                ...
                use_vlm_condition=self.use_vlm_condition, \
                vlm_embedding_dir=self.opt.vlm_embedding_dir, \
                vlm_embedding_text_dir=self.opt.vlm_embedding_text_dir) 

        # init fetch time accumulator
        self._fetch_time_accum = 0.0
        self._fetch_count = 0
    ...
    # def vlm_projector(self, vlm_embedding):
    #     in_dim = vlm_embedding.shape[1]
    #     out_dim = 512 # CHOIS
    #     vlm_embedding_input = nn.Sequential(
    #         nn.Linear(in_features=in_dim, out_features=out_dim),
    #     )(vlm_embedding)

    #     return vlm_embedding_input 
    ...
    def prep_dataloader(self, window_size):
        ...
        train_dataset = CanoObjectTrajDataset(train=True, data_root_folder=self.data_root_folder, \
            ...
            vlm_embedding_dir=self.opt.vlm_embedding_dir,
            vlm_embedding_text_dir=self.opt.vlm_embedding_text_dir,
            use_dual_condition=self.use_dual_condition)
        t1 = time.perf_counter()
        logger.info(f"Dataset(train) initialized in {(t1 - t0):.3f}s, size={len(train_dataset)}")
        ...
        # img_dataset = CanoObjectTrajDataset(train=False, data_root_folder=self.data_root_folder, \
        #     window=window_size, use_object_splits=self.use_object_split, \
        #     input_language_condition=self.add_language_condition, \
        #     use_random_frame_bps=self.use_random_frame_bps, \
        #     use_object_keypoints=self.use_object_keypoints,\
        ...
        logger.info(f"Dataset(val) initialized in {(t3 - t2):.3f}s, size={len(val_dataset)}")

        self.ds = train_dataset 
        self.val_ds = val_dataset
        # Use custom collate_fn when VLM is enabled for proper padding and masking
        # collate_fn = collate_fn_with_vlm_padding if self.use_vlm_condition or self.use_dual_condition else None
        ...
        logger.info(f"DataLoaders created in {(t5 - t4):.3f}s (batch_size={self.batch_size})")
    ...
    def save(self, milestone):
        ...
        print(f"✅ checkpoint saved: model-{milestone}.pt (step: {self.step})")

    def load(self, milestone = None, pretrained_path=None, resume_training=False):
        """
        加载检查点
        Args:
            milestone: 里程碑编号
            pretrained_path: 预训练模型路径
            resume_training: 是否恢复训练状态（包括优化器等）
        """
        ...
        if not resume_training:
            ...
            if missing_keys:
                print(f"⚠️  模型缺失参数: {len(missing_keys)} 个")
            if unexpected_keys:
                print(f"⚠️  模型多余参数: {len(unexpected_keys)} 个")

            self.step = data['step'] +20
            self.model.load_state_dict(data['model'], strict=False)
            self.ema.load_state_dict(data['ema'], strict=False)
            self.scaler.load_state_dict(data['scaler'])
    ...
    def load_pretrained_chois(self, pretrained_path):
        """加载预训练的CHOIS模型参数"""
        print(f"正在加载预训练CHOIS模型: {pretrained_path}")
        try:
            ...
            if 'model' in checkpoint:
                ...
                print(f"加载的checkpoint step: {checkpoint.get('step', 'unknown')}")
            else:
                state_dict = checkpoint
            
            # 加载state dict，允许缺失的key（如vlm_projector相关）
            # - missing_keys: 模型中有但state_dict中没有的键
            # - unexpected_keys: state_dict中有(预训练模型有) 但现在这个模型中没有的键
            missing_keys, unexpected_keys = self.model.load_state_dict(state_dict, strict=False)
            
            # # 检查缺失的key是否只和VLM相关（预期内）
            vlm_missing = [k for k in missing_keys if ('vlm_token_projector' in k or 'vlm_aggregator' in k or 'vlm_transformer_projector' in k)]
            other_missing = [k for k in missing_keys if ('vlm_token_projector' not in k and 'vlm_aggregator' not in k and 'vlm_transformer_projector' not in k)]
            
            if vlm_missing:
                 print(f"缺失VLM projector相关参数（预期）: {len(vlm_missing)} 个")
            
            if other_missing:
                 print(f"⚠️  警告: 缺失非VLM参数: {other_missing}")
                 print("这可能表示模型结构不匹配！")
            
            if unexpected_keys:
                 print(f"⚠️  警告: 多余的参数: {unexpected_keys}")
                 print("这可能表示预训练模型有额外组件")
            
            # # 检查关键组件是否加载
            critical_components = ['denoise_fn', 'bps_encoder', 'clip_encoder']
            for component in critical_components:
                 if any(component in k for k in other_missing):
                     raise ValueError(f"关键组件 '{component}' 未在预训练模型中加载！")
                    
            print("✅ 成功加载预训练CHOIS模型")
            
        except Exception as e:
            print(f"❌ 加载预训练模型出错: {e}")
            raise e

    def freeze_chois_parameters(self):
        """根据vlm_projection_type冻结CHOIS参数，只训练对应的VLM projector模块"""
        frozen_params = 0
        trainable_params = 0
        
        # 获取当前使用的VLM projection类型
        vlm_projection_type = getattr(self.model, 'vlm_projection_type', 'mean_pooling')
        print(f"🔧 VLM projection type: {vlm_projection_type}")
        
        # 根据projection类型确定需要训练的参数
        if vlm_projection_type == "mean_pooling":
            # 原始方案：只训练vlm_aggregator和vlm_token_projector
            trainable_keywords = ['vlm_aggregator', 'vlm_token_projector']
            print("📌 Freezing mode: Only vlm_aggregator and vlm_token_projector will be trainable")
        elif vlm_projection_type == "transformer":
            # 新方案：只训练vlm_transformer_projector
            trainable_keywords = ['vlm_transformer_projector']
            print("📌 Freezing mode: Only vlm_transformer_projector will be trainable")
        else:
            raise ValueError(f"Unknown vlm_projection_type: {vlm_projection_type}")
        ...
        # 验证对应的VLM projector参数是否真的可训练
        trainable_vlm_params = [p for name, p in self.model.named_parameters() 
                               if any(keyword in name for keyword in trainable_keywords) and p.requires_grad]
        
        if len(trainable_vlm_params) == 0:
            print("⚠️  WARNING: No VLM projector parameters found or all are frozen!")
        else:
            # 按模块统计trainable参数
            for keyword in trainable_keywords:
                module_params = [p for name, p in self.model.named_parameters() 
                               if keyword in name and p.requires_grad]
                if module_params:
                    total_params = sum(p.numel() for p in module_params)
                    print(f"✅ {keyword} trainable parameters: {total_params:,}")
                else:
                    print(f"⚠️  {keyword} has no trainable parameters!")
    ...
    def validate_training_setup(self):
        ...
        # 1. Check if VLM condition is enabled
        if not (self.use_vlm_condition or self.use_dual_condition):
            raise ValueError("❌ VLM condition is not enabled! Add --use_vlm_condition or --use_dual_condition flag")
        
        # 2. Check VLM projector components exist based on projection type
        vlm_projection_type = getattr(self.model, 'vlm_projection_type', 'mean_pooling')
        print(f"🔧 VLM projection type: {vlm_projection_type}")
        
        if vlm_projection_type == "mean_pooling":
            vlm_components = ['vlm_token_projector', 'vlm_aggregator']
        elif vlm_projection_type == "transformer":
            vlm_components = ['vlm_transformer_projector']
        else:
            raise ValueError(f"Unknown vlm_projection_type: {vlm_projection_type}")
        ...
        if missing_components:
            raise ValueError(f"❌ VLM projector components not found in model: {missing_components}")
        print(f"✅ VLM projector components found: {', '.join(vlm_components)}")
            
        # 3. Check training mode consistency
        if self.freeze_chois_and_train_projection:
            # Should only have VLM projector components trainable
            all_trainable = [name for name, p in self.model.named_parameters() if p.requires_grad]
            
            # Check for VLM-related parameters
            vlm_trainable = [name for name in all_trainable if any(comp in name for comp in vlm_components)]
            non_vlm_trainable = [name for name in all_trainable if not any(comp in name for comp in vlm_components)]
            
            if non_vlm_trainable:
                print(f"⚠️  WARNING: Non-VLM parameters are trainable: {non_vlm_trainable}")
                
            # Check if VLM components are trainable
            if not vlm_trainable:
                raise ValueError("❌ VLM projector components are frozen in projection-only training mode!")
            
            print(f"✅ Freeze mode: Only VLM projector components are trainable")
            
            # Detailed parameter count for each VLM component
            for component in vlm_components:
                component_params = sum(p.numel() for name, p in self.model.named_parameters() 
                                     if component in name and p.requires_grad)
                print(f"   - {component} trainable params: {component_params:,}")
            
        else:
            print("✅ Full training mode: All parameters are trainable")
            
        # 4. Check optimizer parameters
        optimizer_param_count = sum(p.numel() for group in self.optimizer.param_groups for p in group['params'])
        model_trainable_count = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        
        if optimizer_param_count != model_trainable_count:
            raise ValueError(f"❌ Optimizer parameter count ({optimizer_param_count:,}) != model trainable count ({model_trainable_count:,})")
        
        print(f"✅ Optimizer correctly configured with {optimizer_param_count:,} parameters")
        
        # 5. Check VLM projector method availability
        if not hasattr(self.model, 'vlm_projector') or not callable(getattr(self.model, 'vlm_projector')):
            raise ValueError("❌ VLM projector method not found or not callable!")
        print("✅ VLM projector method is available and callable")
        
        # 6. Check dataset VLM loading
        try:
            sample_data = next(iter(self.dl))
            if 'vlm_embedding' not in sample_data:
                raise ValueError("❌ VLM embeddings not found in dataset!")
            print(f"✅ VLM embeddings found in dataset, shape: {sample_data['vlm_embedding'].shape}")
        except Exception as e:
            print(f"⚠️  WARNING: Could not validate VLM embeddings in dataset: {e}")
            
        print("="*50)
        print("✅ TRAINING SETUP VALIDATION COMPLETE")
        print("="*50 + "\n")
    ...
    def prep_start_end_condition_mask_pos_only(self, data, actual_seq_len):
        ...
        # feature_dimension = data.shape[2]
        # target_shape = (batch_size, sequence_length, feature_dimension) 
        # mask = torch.ones(target_shape, dtype=torch.float32).to(data.device) 
        # mask[:,0,:] = 0
    ...
    def prep_mimic_A_star_path_condition_mask_pos_xy_only(self, data, actual_seq_len):
        ...
        #  # 让路径点掩码全为1 即全都不是条件，不需要考虑waypoints
        # batch_size = data.shape[0]
        # sequence_length = data.shape[1]
        # feature_dimension = data.shape[2]
        # target_shape = (batch_size, sequence_length, feature_dimension) 
    ...
    def train(self):
        ...
        for idx in tqdm(range(init_step, self.train_num_steps), ncols=0):
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                fetch_dt = t_fetch1 - t_fetch0
                self._fetch_time_accum += fetch_dt
                self._fetch_count += 1
                # if self._fetch_count % 20 == 0:
                logger.info(f"Avg. batch data fetch time over last : {self._fetch_time_accum:.4f}s")
                self._fetch_time_accum = 0.0
                self._fetch_count = 0
                ...
                with autocast(enabled = self.amp):    
                    ...
                    # if use VLM embedding, No need for language_condition and clip
                    if not self.add_language_condition and self.use_vlm_condition:
                        vlm_embedding_input = data_dict['vlm_embedding'] 
                        vlm_text_embedding_input = data_dict['vlm_text_embedding']
                        vlm_mask_input = data_dict.get('vlm_mask', None)  # Get VLM mask from collate_fn
                        vlm_text_mask_input = data_dict.get('vlm_text_mask', None)  # Get VLM mask from collate_fn

                        vlm_embedding_input = vlm_embedding_input.to(data.device) # (BS,vlm_shape)
                        vlm_text_embedding_input = vlm_text_embedding_input.to(data.device) # (BS,vlm_shape)
                        vlm_mask_input = vlm_mask_input.to(data.device)  # (BS, seq_len)
                        vlm_text_mask_input = vlm_text_mask_input.to(data.device)  # (BS, seq_len)
                       
                        loss_diffusion, loss_obj, loss_human, loss_feet, loss_fk, loss_obj_pts = \
                        self.model(data, ori_data_cond, cond_mask, padding_mask, \
                        vlm_input=vlm_embedding_input, vlm_mask=vlm_mask_input, \
                        vlm_text_input=vlm_text_embedding_input, vlm_text_mask=vlm_text_mask_input, \
            ...
            if self.step != 0 and self.step % 100 == 0:
                ...
                steps_per_sec =  elapsed / 100
                print(f"完成100步训练用时: {elapsed:.2f}秒 (速度: {steps_per_sec:.2f}秒/步)")
                logger.info(f"完成100步训练用时: {elapsed:.2f}秒 (速度: {steps_per_sec:.2f}秒/步)")
            ...
            if self.step != 0 and self.step % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    if not self.add_language_condition and self.use_vlm_condition:
                        vlm_embedding_input = val_data_dict['vlm_embedding'] 
                        vlm_mask_input = val_data_dict.get('vlm_mask', None)  # Get VLM mask from collate_fn
                        vlm_text_embedding_input = val_data_dict['vlm_text_embedding']
                        vlm_text_mask_input = val_data_dict.get('vlm_text_mask', None)  # Get VLM mask from collate_fn

                        vlm_embedding_input = vlm_embedding_input.to(data.device) 
                        vlm_mask_input = vlm_mask_input.to(data.device)
                        vlm_text_embedding_input = vlm_text_embedding_input.to(data.device)
                        vlm_text_mask_input = vlm_text_mask_input.to(data.device)
                       
                        val_loss_diffusion, val_loss_obj, val_loss_human, val_loss_feet, val_loss_fk, val_loss_obj_pts = \
                        self.model(data, ori_data_cond, cond_mask, padding_mask, \
                        vlm_input=vlm_embedding_input, vlm_mask=vlm_mask_input, \
                        vlm_text_input=vlm_text_embedding_input, vlm_text_mask=vlm_text_mask_input, \
                        rest_human_offsets=rest_human_offsets, ds=self.val_ds, data_dict=val_data_dict)                    
                    else:
    ...
    def cond_sample_res(self):
        if self.opt.pretrained_model == "":
            ...
            self.load(milestone = milestone, resume_training=False)  # 采样时不需要恢复训练状态
        else:
            milestone = "10" # 9, 10
            self.load(milestone = milestone, pretrained_path=self.opt.pretrained_model, resume_training=False) 
        ...
        for s_idx, val_data_dict in enumerate(test_loader):
            ...
            # if os.path.exists(os.path.join(opt.save_res_folder,"single_window_cmp_settings", "chois",seq_name_list[0] + "_" + seq_name_list[0].split("_")[1] + "_sidx_0_eidx_119_sample_cnt_0.mp4")):
            #     continue
            ...
            if not self.add_language_condition and self.use_vlm_condition:
                vlm_embedding_input = val_data_dict['vlm_embedding']
                vlm_mask_input = val_data_dict.get('vlm_mask', None) 
                vlm_text_embedding_input = val_data_dict['vlm_text_embedding']
                vlm_text_mask_input = val_data_dict.get('vlm_text_mask', None) 
            ...
            gt_obj_com_pos_list, gt_obj_rot_mat_list, gt_obj_verts_list, human_faces_list, obj_faces_list, _ , gt_concatenated_data= \
            ...
            pred_obj_com_pos_list, pred_obj_rot_mat_list, pred_obj_verts_list, _, _, _ , concatenated_data= \
            ...
            for tmp_bs_idx in range(tmp_bs):
                ...
                gt_seq_dest_res_npz_path = "/".join(dest_res_for_eval_npz_folder.split("/")[:-1])
                gt_seq_dest_res_npz_path = os.path.join(gt_seq_dest_res_npz_path, "gt")
                if not os.path.exists(gt_seq_dest_res_npz_path):
                    os.makedirs(gt_seq_dest_res_npz_path)                   
                gt_seq_dest_res_npz_path = os.path.join(gt_seq_dest_res_npz_path, \
                                            tmp_seq_name + ".npz")              
                ...
                # np.savez(gt_seq_dest_res_npz_path, seq_name=tmp_seq_name, \
                #         global_jpos=curr_gt_global_jpos) # T X 24 X 3 
                
                if self.test_unseen_objects:
                    save_path = os.path.join(dest_res_for_eval_npz_folder + '_147', tmp_seq_name+"_"+tmp_obj_name+".npy")
                else:
                    save_path = os.path.join(dest_res_for_eval_npz_folder + '_147', tmp_seq_name+".npy")
                if not os.path.exists(dest_res_for_eval_npz_folder+'_147'):
                    os.makedirs(dest_res_for_eval_npz_folder+'_147')                
                np.save(save_path, concatenated_data.detach().cpu().numpy())
    ...
    def cond_sample_res_w_long_planned_path(self):
        if self.opt.pretrained_model == "":
            ...
            self.load(milestone, resume_training=False)  # 采样时不需要恢复训练状态
        else:
            milestone = "10"
            self.load(milestone, pretrained_path=self.opt.pretrained_model, resume_training=False) 
    ...
    def gen_vis_res_generic(self, all_res_list, data_dict, step, cond_mask, vis_gt=False, vis_tag=None, \
        ...
        global_rot_mat = transforms.rotation_6d_to_matrix(global_rot_6d) # N X T X 22 X 3 X 3 
        # pred_seq_com_pos  pred_obj_rot_mat  global_jpos  global_rot_6d
        ...
        for idx in range(num_seq):
            curr_global_rot_mat = global_rot_mat[idx] # T X 22 X 3 X 3 
            curr_local_rot_mat = quat_ik_torch(curr_global_rot_mat) # T X 22 X 3 X 3 
            curr_local_rot = transforms.matrix_to_rotation_6d(curr_local_rot_mat) # T X 22 X 6
            curr_local_rot_aa_rep = transforms.matrix_to_axis_angle(curr_local_rot_mat) # T X 22 X 3 
            ...
            save_root_trans = root_trans
            save_local_rot = curr_local_rot.reshape(-1,22*6)
            save_obj_com_pos = pred_seq_com_pos[idx]
            save_obj_rot = curr_obj_rot_mat.reshape(-1,9)
            concatenated_data = torch.cat([
                save_root_trans,                # (T, 3)
                save_local_rot,          # (T, 22*6)
                save_obj_com_pos,         # (T, 3)
                save_obj_rot        # (T, 9)
            ], dim=1)  # pred_seq_com_posz shape: (T, 3+22*6+3+9) = (T, 147)
            ...
            if move_to_planned_path is not None:
                ...
                # change view: view1
                floor_blend_path = os.path.join(self.data_root_folder, "floor_colorful_mat_view1.blend")
        ...
        obj_verts_list, human_mesh_faces_list, obj_mesh_faces_list, dest_out_vid_path, concatenated_data  
    ...
    def gen_vis_res(self, all_res_list, data_dict, step, cond_mask, vis_gt=False, vis_tag=None, \
        ...
        for idx in range(num_seq):
            ...
            start_obj_com_pos = data_dict['ori_obj_motion'][idx, 0:1, :3] # 1 X 3 
            end_obj_com_pos = data_dict['ori_obj_motion'][idx, actual_len-1:actual_len, :3] # 1 X 3 
            ...
            waypoints_list.append(end_obj_com_pos_xy)
            curr_timesteps = curr_cond_mask.shape[0]
            for t_idx in range(curr_timesteps-1):
                if curr_cond_mask[t_idx] == 0 and t_idx != 0:
                    selected_waypoint = data_dict['ori_obj_motion'][idx, t_idx:t_idx+1, :3]
                    selected_waypoint[:, 2] = 0.05
                    waypoints_list.append(selected_waypoint)
            ...
            if os.path.exists(ball_mesh_save_folder):
                shutil.rmtree(ball_mesh_save_folder)                        
...
def run_train(opt, device):
    ...
    diffusion_model = ObjectCondGaussianDiffusion(opt, d_feats=repr_dim, d_model=opt.d_model, \
                ...
                vlm_projection_type=getattr(opt, 'vlm_projection_type', 'mean_pooling')) 
   
    diffusion_model.to(device)

    # n_parameters = sum(p.numel() for p in diffusion_model.parameters())
    # print(f"Number of params: {n_parameters:,}")
    # n_trainable_parameters = sum(p.numel() for p in diffusion_model.parameters() if p.requires_grad)
    # print(f"Number of trainable params: {n_trainable_parameters:,}")
    # raise
    ...
    trainer = Trainer(
        ...
        train_num_steps=700050,         # 700000, total training steps
    ...
    # 检查是否需要从检查点恢复训练
    if opt.resume_training_path:
        trainer.load(pretrained_path=opt.resume_training_path, resume_training=True)
        print(f"✅ training will start at step{trainer.step} from checkpoint")
...
def parse_opt():
    ...
    parser.add_argument('--resume_training_path', type=str, default="", help='resume training from checkpoint (milestone number)')
    ...
    # vihoi
    parser.add_argument("--use_vlm_condition", action="store_true")

    parser.add_argument("--freeze_chois_and_train_projection", action="store_true")
    parser.add_argument('--pretrained_chois_path', type=str, default="", help='path to pretrained CHOIS model (.pt file)')

    parser.add_argument('--vlm_embedding_dir', type=str, default="vlm_embedding", help='directory for vlm embedding files')
    parser.add_argument('--vlm_embedding_text_dir', type=str, default="vlm_embedding_text", help='directory for vlm embedding files')

    parser.add_argument('--vlm_projection_type', type=str, default='mean_pooling', 
                       choices=['mean_pooling', 'transformer'], 
                       help='VLM projection method: mean_pooling (original) or transformer (cross-attention)')


    # parser.add_argument("--use_dual_condition", action="store_true")
   
    opt = parser.parse_args()
    opt.use_dual_condition = False
...
if __name__ == "__main__":
    ...
    makepath(OUTPUT_PATH, isfile=False)

    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    logger_path = makepath(os.path.join(OUTPUT_PATH, f"{timestamp}-{opt.exp_name}.log"), isfile=True)
    cfg_logger(logger_path)
    logger.info(f"Logging to {logger_path}")
