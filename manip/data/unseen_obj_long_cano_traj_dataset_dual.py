...
class UnseenCanoObjectTrajDataset(Dataset):
    def __init__(
        ...
        if self.use_vlm_condition:
            ...
            # unique_seq_names = sorted(list(set([self.new_window_data_dict[k]['seq_name'] for k in self.new_window_data_dict])))
            hidden_layer_text = self.vlm_embedding_text_dir.split("_")[-2]
            text_embedding_dir = os.path.join(self.vlm_embedding_text_dir + "_test")
    ...
    def generate_data_for_unseen_objects(self):
        ...
        unseen_object_corr_dict = {
            ...
            "smallbox": [ "8dcdf4ef-6969-41af-b203-0bc7c22acb35"]            
        }
        ...
        for k in self.window_data_dict:
            ...
            for unseen_obj_name in unseen_object_list:
                ...
                unseen_obj_com_height = torch.ones(self.window, 1).float() * unseen_obj_com_on_floor[2] # 120 X 1
    ...
    def load_vlm_condition(self, seq_name):
        ...
        embedding_path = os.path.join(self.vlm_embedding_dir + "_test", seq_name + "_" + hidden_layer + "_visual" + ".pt")
            # embedding_text_path = os.path.join(self.vlm_embedding_text_dir + "_test", seq_name + "_" + hidden_layer_text + "_textual" + ".pt")

        if not os.path.exists(embedding_path):
            raise FileNotFoundError(f"VLM embedding not found at: {embedding_path}")
        # if not os.path.exists(embedding_text_path):
        #     raise FileNotFoundError(f"VLM embedding text not found at: {embedding_text_path}")
        ...
        # if len(vlm_text_embedding.shape) == 3:
        #     vlm_text_embedding = np.squeeze(vlm_text_embedding, axis=0) # Remove batch dim if present
