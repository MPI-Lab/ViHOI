...
from manip.data.cano_traj_dataset_dual import quat_fk_torch, quat_ik_torch 

from manip.model.transformer_module_dual import Decoder 
...
class TransformerProjector(nn.Module):
    """
    Transformer-based VLM projector that uses cross-attention to aggregate
    VLM token embeddings into a fixed-size condition vector.
    """
    def __init__(self, d_input=2048, d_output=512, n_head=8, num_layers=2, dropout=0.1):
        super().__init__()
        
        # Ensure d_output is divisible by n_head
        assert d_output % n_head == 0, f"d_output ({d_output}) must be divisible by n_head ({n_head})"
        
        # Learnable query token that serves as an information aggregation point
        self.query_token = nn.Parameter(torch.randn(1, 1, d_output))
        
        # Project VLM embeddings to internal dimension
        self.input_projection = nn.Linear(d_input, d_output)
        
        # Layer normalization for training stability
        self.input_norm = nn.LayerNorm(d_output)
        ...
        self.transformer_layers = nn.ModuleList([
            nn.TransformerDecoderLayer(
                d_model=d_output,
                nhead=n_head,
                dim_feedforward=d_output * 2,  # Reduced FFN dimension for efficiency
                dropout=dropout,
                activation='gelu',
                batch_first=True,
        ...
        # Optional: Position encoding for VLM tokens
        self.use_pos_encoding = False  # Start without position encoding, can enable later
        print(f"use_pos_encoding: {self.use_pos_encoding}")
        if self.use_pos_encoding:
            self.pos_encoding = nn.Parameter(torch.randn(1, 5000, d_output))
        
    def forward(self, vlm_embedding, vlm_mask=None):
        """
        Args:
            vlm_embedding: (BS, seq_len, 2048) - VLM token embeddings
            vlm_mask: (BS, seq_len) - 1 for real tokens, 0 for padding
        
        Returns:
            condition_vector: (BS, 512) - Aggregated condition vector
        """
        bs, seq_len = vlm_embedding.shape[:2]
        
        # 1. Project to internal dimension
        vlm_projected = self.input_projection(vlm_embedding)  # (BS, seq_len, 512)
        vlm_projected = self.input_norm(vlm_projected)
        
        # 2. Add positional encoding (optional)
        if self.use_pos_encoding:
            vlm_projected = vlm_projected + self.pos_encoding[:, :seq_len, :]
        
        # 3. Prepare query token
        query = self.query_token.repeat(bs, 1, 1)  # (BS, 1, 512)
        
        # 4. Prepare attention mask for handling padding
        if vlm_mask is not None:
            # Convert to boolean mask: False for positions that should be ignored
            memory_key_padding_mask = ~vlm_mask.bool()  # (BS, seq_len)
        else:
            memory_key_padding_mask = None
        
        # 5. Apply transformer layers with cross-attention
        for layer in self.transformer_layers:
            query = layer(
                tgt=query,  # Query: (BS, 1, 512)
                memory=vlm_projected,  # Key, Value: (BS, seq_len, 512)
                memory_key_padding_mask=memory_key_padding_mask  # Mask padding tokens
            )
        
        # 6. Return aggregated condition vector
        return query.squeeze(1)  # (BS, 512)
...
class TransformerDiffusionModel(nn.Module):
    ...
    def forward(self, src, noise_t, condition, language_embedding=None, padding_mask=None, vlm_embedding=None,vlm_text_embedding=None):
        # src: BS X T X D 3+9+24*3+22*6+4=220
        # noise_t: int 
        # condition：BS X T X D' 3+9+24*3+22*6+4+256

        # motion sequence + condition
        src = torch.cat((src, condition), dim=-1) # BS X T X (D + D')
       
        noise_t_embed = self.time_mlp(noise_t) # BS X d_model 

        # Combine time and language embedding as one condition token
        obj_embedding = noise_t_embed
        obj_embedding_text = noise_t_embed

        # For obj_embedding: vlm and noise level token
        num_extra_tokens = 0 

        if vlm_embedding is not None and language_embedding is None: # only vlm embedding
            if vlm_embedding.dim() == 2:
            # 如果VLM_token为1，就时间步就与vlm_embedding相加
                obj_embedding += vlm_embedding # BS X d_model  
                vlm_embedding = None
                num_extra_tokens += 1  
            if vlm_text_embedding.dim() == 2:      
                obj_embedding_text += vlm_text_embedding # BS X d_model
                vlm_text_embedding = None
                num_extra_tokens += 1 
        # num_extra_tokens += 1 #方法二
        # 方法一：stage1 文本 stage2 文本+图像  num_extra_tokens = 2
        # 方法二：stage1 文本 stage2 图像  num_extra_tokens = 1       

        obj_embedding = obj_embedding[:, None, :] # BS X 1 X d_model        
        obj_embedding_text = obj_embedding_text[:, None, :] # BS X 1 X d_model
        
        bs = src.shape[0]
        num_steps = src.shape[1] + num_extra_tokens 

        # The padding mask needs to cover the full sequence including new tokens
        if padding_mask is None:
            padding_mask = torch.ones(bs, 1, num_steps).to(src.device).bool() # BS X 1 X num_steps
        else:
            # The provided padding_mask has shape (bs, 1, T+1).
            # take the part for the motion sequence (last T elements)
            # prepend the pads for our new condition tokens
            motion_sequence_mask = padding_mask[:, :, 1:] # Shape: (bs, 1, T)  
            extra_pads = torch.ones(bs, 1, num_extra_tokens, device=src.device).bool() # Shape: (bs, 1, num_extra_tokens)
            padding_mask = torch.cat((extra_pads, motion_sequence_mask), dim=2) # Shape: (bs, 1, T+num_extra_tokens)

        # Get position vec for position-wise embedding, length must match the new sequence length
        pos_vec = torch.arange(num_steps) + 1 # timesteps
        pos_vec = pos_vec[None, None, :].to(src.device).repeat(bs, 1, 1) # BS X 1 X timesteps

        # ---- DEBUG PRINTS ----
        # print(f"--- Debug Info: TransformerDiffusionModel.forward ---")
        # print(f"src shape: {src.shape}")
        # if vlm_embedding is not None:
        #     print(f"vlm_embedding shape: {vlm_embedding.shape}")
        # print(f"obj_embedding shape: {obj_embedding.shape}")
        # print(f"obj_embedding_text shape: {obj_embedding_text.shape}")
        # print(f"num_extra_tokens: {num_extra_tokens}")
        # print(f"num_steps (total sequence length): {num_steps}")
        ...
        feat_pred, _ = self.motion_transformer(data_input, padding_mask, pos_vec, obj_embedding=obj_embedding, obj_embedding_text=obj_embedding_text, motion_sequence_mask=motion_sequence_mask)
       
        # print(f"feat_pred shape from transformer: {feat_pred.shape}")

        output = self.linear_out(feat_pred[:, num_extra_tokens:]) # BS X T X D
...
class ObjectCondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        vlm_projection_type="mean_pooling",  # "mean_pooling" or "transformer"
    ):
        super().__init__()

        # embedding from VLM
        input_embedding_dim = d_model # d_model
        
        # VLM projection type selection
        self.vlm_projection_type = vlm_projection_type
        
        if vlm_projection_type == "mean_pooling":
            # Original implementation: token-wise projection + masked mean pooling
            self.vlm_token_projector = nn.Sequential(
                nn.Linear(in_features=2048, out_features=256),
                nn.ReLU(),
                nn.Dropout(0.1)
            )
            self.vlm_aggregator = nn.Sequential(
                nn.Linear(in_features=256, out_features=input_embedding_dim),
            )
            # Additional projector for vlm_text_embedding 
            self.vlm_text_token_projector = nn.Sequential(
                nn.Linear(in_features=2048, out_features=256),
                nn.ReLU(),
                nn.Dropout(0.1)
            )
            self.vlm_text_aggregator = nn.Sequential(
                nn.Linear(in_features=256, out_features=input_embedding_dim),
            ) 

        elif vlm_projection_type == "transformer":
            # New transformer-based implementation: cross-attention aggregation
            self.vlm_transformer_projector = TransformerProjector(
                d_input=2048,
                d_output=input_embedding_dim,  # 512
                n_head=8,
                num_layers=2,
                dropout=0.1
            )
            # Additional transformer projector for vlm_text_embedding 
            self.vlm_text_transformer_projector = TransformerProjector(
                d_input=2048,
                d_output=input_embedding_dim,  # 512
                n_head=8,
                num_layers=2,
            ...
            raise ValueError(f"Unknown vlm_projection_type: {vlm_projection_type}. "
                           f"Supported types: 'mean_pooling', 'transformer'")
        
        # Alternative implementation using attention-based aggregation:
        # self.vlm_sequence_projector = nn.Linear(2048, 256)  # Project each token
        # self.vlm_attention = nn.MultiheadAttention(embed_dim=256, num_heads=8, batch_first=True)
        # self.vlm_final_projector = nn.Linear(256, input_embedding_dim)
        # 
        # In forward pass, you would use:
        # vlm_tokens = self.vlm_sequence_projector(vlm_input)  # BS x 3105 x 256
        # vlm_attended, _ = self.vlm_attention(vlm_tokens, vlm_tokens, vlm_tokens)  # BS x 3105 x 256
        # vlm_embedding = vlm_attended.mean(dim=1)  # BS x 256 (global average pooling)
        # vlm_embedding = self.vlm_final_projector(vlm_embedding)  # BS x 512
    ...
    def vlm_projector(self, vlm_input, vlm_mask=None):
        """
        Process VLM input of shape BS x seq_len x 2048 to output BS x 512
        Supports two projection methods: mean_pooling and transformer
        
        Args:
            vlm_input: BS x seq_len x 2048 (padded)
            vlm_mask: BS x seq_len (1 for real tokens, 0 for padding)
        
        Returns:
            vlm_embedding: BS x 512
        """
        if self.vlm_projection_type == "mean_pooling":
            # Original implementation: token-wise projection + masked mean pooling
            bs, seq_len, feat_dim = vlm_input.shape
            
            # Process each token: BS x seq_len x 2048 -> BS x seq_len x 256
            vlm_tokens = self.vlm_token_projector(vlm_input)
            
            # Aggregate across sequence dimension using masked mean pooling
            if vlm_mask is not None:
                # Apply mask: set padding tokens to 0
                vlm_mask_expanded = vlm_mask.unsqueeze(-1).float()  # BS x seq_len x 1
                masked_tokens = vlm_tokens * vlm_mask_expanded  # BS x seq_len x 256
                
                # Calculate mean only over non-padding tokens
                valid_token_counts = vlm_mask.sum(dim=1, keepdim=True).float()  # BS x 1
                vlm_pooled = masked_tokens.sum(dim=1) / valid_token_counts.clamp(min=1)  # BS x 256
            else:
                # Fallback to regular mean pooling if no mask provided
                vlm_pooled = vlm_tokens.mean(dim=1)
            
            # Final projection: BS x 256 -> BS x 512
            vlm_embedding = self.vlm_aggregator(vlm_pooled)
            
        elif self.vlm_projection_type == "transformer":
            # New transformer-based implementation: cross-attention aggregation
            vlm_embedding = self.vlm_transformer_projector(vlm_input, vlm_mask)
            
        else:
            raise ValueError(f"Unknown vlm_projection_type: {self.vlm_projection_type}")
        
        return vlm_embedding 

    def vlm_text_projector(self, vlm_text_input, vlm_text_mask=None):
        """
        Process VLM text input of shape BS x seq_len x 2048 to output BS x 512
        Supports two projection methods: mean_pooling and transformer
        
        Args:
            vlm_text_input: BS x seq_len x 2048 (padded)
            vlm_text_mask: BS x seq_len (1 for real tokens, 0 for padding)
        
        Returns:
            vlm_text_embedding: BS x 512
        """
        if self.vlm_projection_type == "mean_pooling":
            # Text implementation: token-wise projection + masked mean pooling
            bs, seq_len, feat_dim = vlm_text_input.shape
            
            # Process each token: BS x seq_len x 2048 -> BS x seq_len x 256
            vlm_text_tokens = self.vlm_text_token_projector(vlm_text_input)
            
            # Aggregate across sequence dimension using masked mean pooling
            if vlm_text_mask is not None:
                # Apply mask: set padding tokens to 0
                vlm_text_mask_expanded = vlm_text_mask.unsqueeze(-1).float()  # BS x seq_len x 1
                masked_text_tokens = vlm_text_tokens * vlm_text_mask_expanded  # BS x seq_len x 256
                
                # Calculate mean only over non-padding tokens
                valid_token_counts = vlm_text_mask.sum(dim=1, keepdim=True).float()  # BS x 1
                vlm_text_pooled = masked_text_tokens.sum(dim=1) / valid_token_counts.clamp(min=1)  # BS x 256
            else:
                # Fallback to regular mean pooling if no mask provided
                vlm_text_pooled = vlm_text_tokens.mean(dim=1)
            
            # Final projection: BS x 256 -> BS x 512
            vlm_text_embedding = self.vlm_text_aggregator(vlm_text_pooled)
            
        elif self.vlm_projection_type == "transformer":
            # Text transformer-based implementation: cross-attention aggregation
            vlm_text_embedding = self.vlm_text_transformer_projector(vlm_text_input, vlm_text_mask)
            
        else:
            raise ValueError(f"Unknown vlm_projection_type: {self.vlm_projection_type}")
        
        return vlm_text_embedding
    ...
    def p_mean_variance(self, x, t, x_cond, language_embedding=None, padding_mask=None, clip_denoised=True, vlm_embedding=None, vlm_text_embedding=None):
        # x_all = torch.cat((x, x_cond), dim=-1)
        # model_output = self.denoise_fn(x_all, t)

        model_output = self.denoise_fn(x, t, x_cond, language_embedding = None, padding_mask = padding_mask, vlm_embedding = vlm_embedding, vlm_text_embedding=vlm_text_embedding)
    ...
    def p_mean_variance_reconstruction_guidance(self, x, t, x_cond, guidance_fn, opt_fn=None, \
                                    ...
                                    clip_denoised=True, vlm_embedding=None, vlm_text_embedding=None):
        # x_all = torch.cat((x, x_cond), dim=-1)
        # model_output = self.denoise_fn(x_all, t)
        with torch.enable_grad():
            ...
            if vlm_embedding is not None :
                model_output = self.denoise_fn(src=x, noise_t=t, condition=x_cond, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding, padding_mask=padding_mask)
    ...
    def p_sample_guided_reconstruction_guidance(self, x, t, x_cond, guidance_fn, language_embedding=None, \
                    ...
                    contact_labels=None, curr_window_ref_obj_rot_mat=None, vlm_embedding=None, vlm_text_embedding=None):
        ...
        if use_reconstruction_guidance:
            ...
            model_mean, _, model_log_variance = self.p_mean_variance_reconstruction_guidance(x=x, t=t, x_cond=x_cond, \
                                        ...
                                        contact_labels=contact_labels, \
                                        curr_window_ref_obj_rot_mat=curr_window_ref_obj_rot_mat, \
                                        vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)  
            
            new_mean = model_mean 
        else: # Classifier-guidance 
            model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, x_cond=x_cond, \
                language_embedding=language_embedding, \
                padding_mask=padding_mask, clip_denoised=clip_denoised, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)
    ...
    def p_sample_loop_guided(self, shape, x_cond, guidance_fn=None, language_embedding=None, opt_fn=None, \
                    ...
                    cond_mask=None, padding_mask=None, vlm_embedding=None, vlm_text_embedding=None):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            if guidance_fn is not None and i > 0 and i < 10: 
                x = self.p_sample_guided_reconstruction_guidance(x, torch.full((b,), i, device=device, dtype=torch.long), \
                            ...
                            cond_mask=cond_mask, padding_mask=padding_mask, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)  
                ...
                x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), x_cond, \
                    language_embedding, padding_mask=padding_mask, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)    

        return x # BS X T X D

    @torch.no_grad()
    def p_sample(self, x, t, x_cond, language_embedding=None, padding_mask=None, clip_denoised=True, vlm_embedding=None,vlm_text_embedding=None):
        b, *_, device = *x.shape, x.device
    ...
    def p_sample_loop(self, shape, x_cond, language_embedding=None, padding_mask=None, \
                cond_mask=None, ori_pose_cond=None, return_diff_level_res=False, vlm_embedding=None, vlm_text_embedding=None):
        device = self.betas.device
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            padding_mask=padding_mask, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)    
    ...
    def sample(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None, \
            ...
            data_dict=None, guidance_fn=None, opt_fn=None, return_diff_level_res=False, vlm_input=None, vlm_mask=None, vlm_text_input=None, vlm_text_mask=None):
        ...
        if vlm_input is not None:
            vlm_embedding = self.vlm_projector(vlm_input, vlm_mask)
        else:
            vlm_embedding = None 

        if vlm_text_input is not None:
            vlm_text_embedding = self.vlm_text_projector(vlm_text_input, vlm_text_mask)
        else:
            vlm_text_embedding = None 
    ...
    def p_losses(self, x_start, x_cond, t, language_embedding=None, noise=None, \
        padding_mask=None, rest_human_offsets=None, data_dict=None, ds=None, vlm_embedding=None, vlm_text_embedding=None):
        # x_start: BS X T X D
        ...
        if language_embedding is None and vlm_embedding is not None:
            model_out = self.denoise_fn(src=x, noise_t=t, condition=x_cond, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding, padding_mask=padding_mask)
    ...
    def forward(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None, \
        language_input=None, contact_labels=None, rest_human_offsets=None, data_dict=None, ds=None, vlm_input=None, vlm_mask=None, vlm_text_input=None, vlm_text_mask=None): 
        ...
        # VLM embedding input dimension conversion
        if vlm_input is not None:
            vlm_embedding = self.vlm_projector(vlm_input, vlm_mask) # BS X d_model 
        else:
            vlm_embedding = None 

        if vlm_text_input is not None:
            vlm_text_embedding = self.vlm_text_projector(vlm_text_input, vlm_text_mask) # BS X d_model 
        else:
            vlm_text_embedding = None 
        ...
        if self.use_object_keypoints:
            curr_loss, curr_loss_obj, curr_loss_human, curr_loss_feet, curr_loss_fk, curr_loss_obj_pts = \
                        ...
                        language_embedding=language_embedding, padding_mask=padding_mask, \
                        rest_human_offsets=rest_human_offsets, data_dict=data_dict, ds=ds, vlm_embedding=vlm_embedding, vlm_text_embedding=vlm_text_embedding)  
