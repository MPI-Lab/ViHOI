...
if __name__ == "__main__":
    ...
    parser.add_argument('--scene', type=str, metavar='PATH',
                        ...
                        default='')
    parser.add_argument('--camera-pos', nargs=3, type=float, default=None, help='Camera position in floats (x y z)')
    parser.add_argument('--camera-rot', nargs=3, type=float, default=None, help='Camera rotation in degrees (x y z)')
    parser.add_argument('--render-frames', nargs='+', type=int, default=None, help='List of frame indices to render')
    ...
    if args.camera_pos is not None and args.camera_rot is not None:
        camera = bpy.data.objects.get("Camera")
        if camera:
            camera.location = args.camera_pos
            camera.rotation_euler = [math.radians(angle) for angle in args.camera_rot]
        else:
            print("Warning: Camera not found in the scene.")
    ...
    if render_frames is None:
        render_frames = range(len(obj_files))

    for frame_idx in render_frames:
        file_name = "%05d.ply" % frame_idx

        # Check if the file for the current frame_idx exists
        if file_name not in obj_files:
            print(f"File {file_name} not found, skipping frame {frame_idx}")
            continue
        ...
        bpy.data.scenes['Scene'].render.filepath = os.path.join(output_dir, ("%05d"%frame_idx)+".png")
