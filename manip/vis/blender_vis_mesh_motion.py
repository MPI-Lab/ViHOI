...
BLENDER_PATH = "/data/songjin/blender-3.4.0-linux-x64/blender" # Put your blender path here 
...
def images_to_video_w_imageio(img_folder, output_vid_file, fps=30):
    ...
    if len(im_arr) == 0:
        print(f"[WARN] No images in {img_folder}, skip writing video (Blender may have failed).")
        return
...
def run_blender_rendering_and_save2video(obj_folder_path, out_folder_path, out_vid_path, condition_folder=None, \
    ...
    camera_pos=None, camera_rot=None, contact_frames=None):
    ...
    if camera_pos is not None and camera_rot is not None:
        camera_command = f" --camera-pos {' '.join(map(str, camera_pos))} --camera-rot {' '.join(map(str, camera_rot))}"

    frame_command = ""
    if contact_frames is not None:
        frame_command = f" --render-frames {' '.join(map(str, contact_frames))}"
    ...
    if vis_object:
        if vis_human: # vis both human and object
            ...
            " -b -- --folder "+obj_folder_path+" --condition-folder "+condition_folder+" --scene "+scene_blend_path+" --out-folder "+out_folder_path + camera_command + frame_command, shell=True) 
            else:
                blender_utils_path = os.path.join(BLENDER_UTILS_ROOT_FOLDER, "blender_vis_utils.py") 
                subprocess.call(BLENDER_PATH+" -P "+blender_utils_path+ \
            " -b -- --folder "+obj_folder_path+" --scene "+scene_blend_path+" --out-folder "+out_folder_path + camera_command + frame_command, shell=True) 
        ...
        if vis_condition:
            ...
            scene_blend_path+" --out-folder "+out_folder_path+" --material-color "+mat_color + camera_command + frame_command, shell=True)    
...
def save_verts_faces_to_mesh_file_w_object(mesh_verts, mesh_faces, obj_verts, obj_faces, save_mesh_folder, contact_frames=None):
    # mesh_verts: T X Nv X 3 
    # mesh_faces: Nf X 3 
    if not os.path.exists(save_mesh_folder):
        os.makedirs(save_mesh_folder)
    ...
    frame_indices = range(num_meshes)
    if contact_frames is not None:
