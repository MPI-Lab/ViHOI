...
#     # 获取批次大小 B 和关节数 N
#     B, N, _, _ = pose.shape
#     # 将输入 reshape 为 (B*N, 3, 3)
#     reshaped_input = pose.reshape(B * N, 3, 3)
#     # 转换为四元数
#     quat = t3d.matrix_to_quaternion(reshaped_input)
#     # 四元数转轴角
#     axis_angle = t3d.quaternion_to_axis_angle(quat)
#     # 将结果 reshape 为 (B, N*3)，即每个样本展平为一个向量
#     return axis_angle.reshape(B, -1)
...
#     # 处理输入维度
#     if len(pose.shape) < 2:
#         pose = pose.unsqueeze(0)  # 添加批次维度
    
#     # 获取批次大小B和总维度数D
#     B, D = pose.shape
#     N = D // 3  # 计算关节数量
    
#     # 将输入reshape为 (B*N, 3) 格式
#     reshaped_input = pose.reshape(B * N, 3)
    
#     # 转换为旋转矩阵 (B*N, 3, 3)
#     rotmat = t3d.axis_angle_to_matrix(reshaped_input)
    
#     # 最终reshape为 (B, N, 3, 3)
#     return rotmat.reshape(B, N, 3, 3)
...
def smplx_loc2glob(local_pose):
    ...
    for i in range(1,len(smplx_parents)):
        pose = global_pose[:,i].clone()
        parent = global_pose[:,smplx_parents[i]].clone()
        global_pose[:,i] = torch.matmul(parent, pose)
